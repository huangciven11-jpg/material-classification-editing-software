...
> 面向电商短视频素材的桌面端工作台：导入、分类、粗剪、版本调整、导出，并支持 API 驱动的文案建议与标签增强。
...
### 专业版增强
- 本地保存 API 配置：`provider / baseUrl / model / apiKey`
- 基于脚本生成文案建议
- 一键复制文案建议
- 基于脚本和素材列表生成标签增强建议
- 逐条应用标签到素材库
...
### 当前状态
- 桌面端技术栈：Electron + React + Vite + TypeScript
- 本地主链已可运行
- 已通过：`npm test` / `npm run build` / `npm run build:electron` / `npm run lint`
...
### 专业版增强流程
1. 在设置页填写 `provider / baseUrl / model / apiKey`
2. 在生成页获取文案建议并复制使用
3. 在生成页获取标签增强建议
4. 逐条应用标签或一键全部应用
5. 回到素材流程继续生成、调整和导出
